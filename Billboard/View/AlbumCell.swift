...
class AlbumCell: UITableViewCell {
    ...
    func setLayout() {
        // Anchors
        self.albumThumbnailImage.topAnchor.constraint(equalTo: self.contentView.topAnchor, constant: 8.0)
            .isActive = true
        self.albumThumbnailImage.leadingAnchor.constraint(equalTo: self.contentView.leadingAnchor,
        ...
        self.albumName.leftAnchor.constraint(equalTo: self.albumThumbnailImage.rightAnchor, constant: 8.0)
            .isActive = true
        self.albumName.topAnchor.constraint(equalTo: self.contentView.topAnchor, constant: 10.0)
            .isActive = true
        self.artistName.leadingAnchor.constraint(equalTo: self.albumName.leadingAnchor).isActive = true
        self.artistName.topAnchor.constraint(equalTo: self.albumName.bottomAnchor, constant: 10.0)
            .isActive = true
        // Bottom constraint with low priority to skip warnings in console
        let bottom = self.albumThumbnailImage.bottomAnchor.constraint(equalTo: self.contentView.bottomAnchor,
                                                                      constant: -10.0)
